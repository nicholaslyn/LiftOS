// LiftOS — robust event handling, no libs, localStorage-only
document.addEventListener("DOMContentLoaded", () => {
  ...
  // delegate clicks inside exerciseWrap (so buttons always work)
  exerciseWrap.addEventListener("click", (ev) => {
    ...
    const rpe    = +card.querySelector(".in-rpe").value    || null;
    ...
    list.insertAdjacentHTML("afterbegin", `<li>${fmtDateTime(row.dateISO)} — <strong>${row.weight}${row.unit} × ${row.reps}</strong>${row.rpe?` @RPE ${row.rpe}`:""} · e1RM ${round1(pr.e1RM)}${row.unit}</li>`);
  ...
  function beep(){ try{
    ...
    g.gain.value=.05; o.start(); setTimeout(()=>{o.stop();ctx.close();},250);
  }catch{} }
  ...
  function platePlan(target, barWeight = settings.bar){
    ...
    let remain = per; const used = []; const avail = [...settings.plates].sort((a,b)=>b-a);
    for (const p of avail){
      let c=0; while(remain + 1e-6 >= p){ remain = round2(remain - p); c++; }
      if (c) used.push({p:c});
    }
    const matched = Math.abs(remain) < 0.01;
    const html = `<div class="line"><span class="plate small">Bar ${barWeight}${esc(settings.unit)}</span> ${
      used.map((u,i)=>`<span class="plate">${Object.keys(u)[0]}${esc(settings.unit)} × ${Object.values(u)[0]} / side</span>`).join("")
    } ${matched?"":`<span class="muted">(≈ off by ${round2(remain)}${esc(settings.unit)} per side)</span>`}</div>`;
    return { html };
  }
